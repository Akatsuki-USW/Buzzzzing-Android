...
class SnsLoginButton @JvmOverloads constructor(
    ...
    var onThrottleClickLoginButton: () -> Unit = {}
        set(value) {
            field = value
            binding.root.onThrottleClick { field() }
        }
    ...
    var state: SnsLoginButtonState = SnsLoginButtonState.Enable
        set(value) {
            field = value
            when(field) {
                SnsLoginButtonState.Enable -> { changeStateToEnable() }
                SnsLoginButtonState.Loading -> { changeStateToLoading() }
            }
        }
    ...
    init {
        ...
        val typedArray = context.obtainStyledAttributes(attrs, R.styleable.SnsLoginButton, defStyleAttr, 0)

        val background = typedArray.getResourceId(R.styleable.SnsLoginButton_loginButtonBackground, R.drawable.ripple_bg_solid_kakao01_rounded_5)
        val progressBarTint = typedArray.getResourceId(R.styleable.SnsLoginButton_loginButtonProgressBarTint, R.color.black01)
        val icon = typedArray.getResourceId(R.styleable.SnsLoginButton_loginButtonSnsLogo, R.drawable.ic_kakao)
        val text = typedArray.getText(R.styleable.SnsLoginButton_loginButtonText)
        val textColor = typedArray.getColor(R.styleable.SnsLoginButton_loginButtonTextColor, ContextCompat.getColor(context, R.color.black01))

        binding.apply {
            constraintLayoutButton.setBackgroundResource(background)
            progressBarLoadingButton.setIndicatorColor(ContextCompat.getColor(context, progressBarTint))
